    ...
    .values([
      {
        ...
        url: "https://r.test/issues/114518",
        rawJson: "{}",
      },
      {
        id: 115498,
        ...
        projectId: 1,
        projectName: "P",
        subject: "Inny earning",
        statusName: "Open",
        createdOn: "2026-04-10T00:00:00Z",
        updatedOn: "2026-04-10T00:00:00Z",
        url: "https://r.test/issues/115498",
        rawJson: "{}",
      },
    ])
    .run();
  ...
  it("posts issue + time entries + relations to Redmine and mirrors locally", async () => {
    ...
    server = startMsw(
      http.get("https://r.test/users/current.json", () =>
        HttpResponse.json({
          user: { id: 1039, login: "oskar.bialek", firstname: "Oskar", lastname: "Białek" },
        }),
      ),
    ...
    const res = await app.request("/api/redemptions/create", {
      ...
      body: JSON.stringify({
        ...
        allocations: [
          ...
          { earningId: 115498, hours: 4 },
        ],
      }),
    });
    expect(res.status).toBe(201);
    const json = (await res.json()) as {
      data: { issueId: number; subject: string; warning?: string };
    };
    ...
    const issueBody = seen.issues[0] as {
      issue: { subject: string; description: string; project_id: number; tracker_id: number };
    };
    expect(issueBody.issue.subject).toBe("Odbiór nadgodzin OB 04.05");
    expect(issueBody.issue.description).toBe(
      "Odbiór 4h z #114518 (R&D - support migracji)\nOdbiór 4h z #115498 (Inny earning)",
    );
    ...
    server = startMsw(
      ...
      http.post("https://r.test/time_entries.json", async ({ request }) => {
        ...
        return HttpResponse.json(
          {
            time_entry: {
              id: 1000 + teCalls.length,
              user: { id: 1039 },
              issue: { id: 999 },
              hours: body.time_entry.hours,
              activity: { id: 8, name: "W biurze" },
              spent_on: body.time_entry.spent_on,
              comments: "",
              created_on: "x",
              updated_on: "x",
            },
